...
class VisualTextFinetuneModelClip(BaseRecognizer):
    ...
    #can not run! Need fix
    @auto_fp16(apply_to=('imgs'))
    def forward_dummy(self, imgs, label=None, token_ids=None, segment_ids=None, input_mask=None, softmax=False):
        ...
        visual_token, visual_cls = self.extract_visual_feat(imgs)
        if self.task == 'zero_shot_clf':
            text_mask_cls = torch.rand((400,512)).type_as(visual_cls)
            text_out_with_mask = torch.rand((400, 32, 512)).type_as(visual_cls)
            input_mask = torch.rand((400, 32)).type_as(visual_cls)
            if self.interaction_head is not None:
                sim_mt = self.interaction_head(text_out_with_mask, visual_cls, input_mask, video_patch=visual_token)
                if hasattr(self.interaction_head, 'reco2'):
                    sim_mt = sim_mt
                else:
                    sim_mt = sim_mt.T
        ...
        if self.task == 'retrieval_ssl':
            text_out_with_mask, text_mask_cls = self.extract_text_feat(token_ids, input_mask)
            outs = self.loss_func(visual_cls, text_mask_cls)
        if softmax:
